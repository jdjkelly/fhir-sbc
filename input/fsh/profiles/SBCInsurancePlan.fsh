...
* identifier ^definition = "Unique identifier for the plan, such as the 14-16 character HIOS ID (5-digit Issuer ID + 2-character State Abbreviation + 3-digit Product Iteration + 4-digit Plan Iteration + optional 2-digit Variant)"
...
* coverage ^definition = "Details of benefit coverage for different categories of services. Each coverage entry should represent one or more of the 30 required SBC benefit categories"

* coverage.type 1..1 MS
* coverage.type ^short = "Type of coverage (e.g., medical, pharmacy)"
* coverage.type ^definition = "High-level categorization of coverage type. Use codes from an appropriate coverage type code system such as v3-ActCode (e.g., HIP for health insurance plan policy)"
...
* coverage.benefit ^definition = "Individual benefit types within this coverage category, corresponding to the 30 SBC benefit categories"
...
* coverage.benefit.type ^definition = "The specific benefit category from the SBC's 30 required categories"
...
* plan ^definition = "Specific plan offerings with detailed cost-sharing information for each of the 30 SBC benefit categories"
...
* plan.specificCost ^definition = "Cost-sharing information (copays, coinsurance, deductibles) for each of the 30 SBC benefit categories, with separate amounts for in-network and out-of-network services"
